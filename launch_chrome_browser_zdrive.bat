...
set temp=Z:\z_CourseReview\04_QA_Review\working\try-%INPUT%.htm
IF EXIST %temp% (set course=Z:\z_CourseReview\04_QA_Review\working\try-%INPUT%.htm
         set FLAG=yes) 
set temp=c:\xml\production\working\try-OLP-%INPUT%.htm
IF EXIST %temp% (set course=Z:\z_CourseReview\04_QA_Review\working\try-OLP-%INPUT%.htm
         set FLAG=yes)
set sharedservertemp=d:\xml\production\working\%USERNAME%\try-OLP-%INPUT%.htm
IF EXIST %sharedservertemp% (set course=Z:\z_CourseReview\04_QA_Review\working\try-OLP-%INPUT%.htm
         set FLAG=yes)
IF %INPUT% EQU preview (set FLAG=yes) 

IF %FLAG% EQU no (msg * try htm not present for the course in Z:\z_CourseReview\04_QA_Review\working
          Exit)
