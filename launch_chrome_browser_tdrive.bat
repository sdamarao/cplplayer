...
set temp=T:\xml\production\working\try-%INPUT%.htm
IF EXIST %temp% (set course=T:\xml\production\working\try-%INPUT%.htm
         set FLAG=yes)
set sharedservertemp=d:\xml\production\working\%USERNAME%\try-%INPUT%.htm
IF EXIST %sharedservertemp% (set course=T:\xml\production\working\try-%INPUT%.htm
         set FLAG=yes)		 
set temp=c:\xml\production\working\try-OLP-%INPUT%.htm
IF EXIST %temp% (set course=T:\xml\production\working\try-OLP-%INPUT%.htm
         set FLAG=yes)
set sharedservertemp=d:\xml\production\working\%USERNAME%\try-OLP-%INPUT%.htm
IF EXIST %sharedservertemp% (set course=T:\xml\production\working\try-OLP-%INPUT%.htm
         set FLAG=yes)
IF %INPUT% EQU preview (set FLAG=yes) 

IF %FLAG% EQU no (msg * try htm not present for the course in T:\xml\production\working
          Exit)
